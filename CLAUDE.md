...
This file supplements `AGENTS.md` for automated development in this repository.
Treat `AGENTS.md` as the canonical project instructions and command reference.
...
bun run build            # Prisma generate + artifact-only Next.js build
bun run vercel:build     # Production migrations + Next.js build
bun run lint             # Biome check
bun run typecheck        # Next route type generation + TypeScript
...
bun run test:coverage:unit           # Unit tests with enforced coverage
bun run test:integration             # Ephemeral-Neon integration suite
bun run test:coverage:integration    # Integration suite with coverage
bun run test:e2e                     # Guest browser suite on ephemeral Neon
bun run test:all                     # Unit and integration coverage, once each
bun run verify                       # Lint, typecheck, and unit coverage
...
Use `bun run` over `npm run` and `bunx` over `npx` unless a tool explicitly
requires npm/npx.
...
- `src/app/api/` — API route handlers and webhooks
...
- **`session-manager.ts`** — Session boundaries and rolling context
...
The runtime uses Vercel AI SDK 7 through OpenRouter. The production orchestrator
model is `openai/gpt-5.6-luna` with OpenRouter priority service and a DeepSeek
fallback. Verify `src/lib/ai/providers/openrouter.ts` and the model catalog before
...
Plan-based limits are enforced per user and organization. Configuration lives in
`config.ts`, enforcement in `check.ts`, and usage is persisted to `DailyUsage`.
...
Local integration and E2E commands require `NEON_API_KEY`, `NEON_PROJECT_ID`,
and a `DATABASE_URL` pointing to the non-default Neon `development` branch. The
runners create an expiring branch, run migrations, inject `TEST_DATABASE_URL`
only into the child process, and delete the branch afterward. Never persist
`TEST_DATABASE_URL` in an environment file.

## Key Conventions

- **Path alias:** `@/*` maps to `./src/*`
- **Linter/formatter:** Biome (not ESLint/Prettier) — 2-space indent, auto import organization
- **Tests:** Unit tests in `*.test.ts`/`*.test.tsx` beside source files; integration tests in `*.integration.test.ts` under `src/app/api/` or `src/lib/`
...
- **API routes:** Use `Request` unless the local route requires `NextRequest`
