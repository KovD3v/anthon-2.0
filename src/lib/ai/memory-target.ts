...
const COACHING_CONTINUATION =
  /(?:\be\b|\band\b|[.!?,;:\u2014-])\s*(?:(?:poi|then)\s+)?(?:(?:prova|proviamo|cerca|cerchiamo|try)\s+(?:a|di|to)\s+)?(?:concentr(?:ati|arti|arsi|arci|iamoci)|focalizz(?:ati|arti|arsi|arci|iamoci)|ripart(?:i|ire|iamo)|ricominci\w*|ricomincer\w*|pens(?:a(?:\b|re\b)|iamo\b)|guarda\s+avanti\b|guardiamo\s+avanti\b|vai\s+avanti\b|andiamo\s+avanti\b|focus\b|refocus\b|restart\b|think\b|move\s+on\b)/i;
