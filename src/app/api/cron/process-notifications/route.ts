	...
	try {
		...
		for (const notification of pendingNotifications) {
			try {
				...
				if (
					lastUserMessage &&
					notification.type === "PROMISED_FOLLOWUP"
				) {
...
async function sendWebPush(notification: any) {
	const subscriptions = notification.user.pushSubscriptions;
	if (!subscriptions.length) return false;

	const payload = JSON.stringify({
		title:
			notification.type === "PROMISED_FOLLOWUP" ? "Hey!" : "Coach Anthon",
...
async function sendWhatsAppPush(notification: any) {
...
async function sendTelegramPush(notification: any) {
