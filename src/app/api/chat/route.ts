...
export async function POST(request: Request) {
	...
	try {
		...
		let user = await LatencyLogger.measure(
			"DB: Find/create user",
			async () => {
		...
		const result = await streamChat({
			...
			onFinish: async ({ text, metrics }) => {
				const finishTimer = LatencyLogger.start(
					...
					try {
						...
						waitUntil(
							extractAndSaveMemories(
								...
								console.error(
									"[Chat API] Memory extraction error:",
									err
								);
							})
						);
