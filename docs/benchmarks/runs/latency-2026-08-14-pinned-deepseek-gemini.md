...
The provider was pinned and confirmed in every response: Google AI Studio, DeepSeek, and CoreWeave respectively. No request was allowed to fall back to another provider.
...
All 15 requests returned HTTP 200. `firstAnyMs` and `firstContentMs` are kept separate: the former includes reasoning deltas, while the latter is the first visible text delta.
